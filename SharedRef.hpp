...
// Not a full mirror of the UE4 implementation, this is not thread safe
// This code was not copy-pasted, but typed out by hand therefore does not infringe
// the UE4 license
#include <cstdint> 
...
public:
    ...
    // Must be initialised with a valid object at construction time

    // Constructs a shared reference that owns the specified object
    // Cannot be nullptr
    template <class OtherType>
    inline explicit SharedRef(OtherType* InObject)
        : Object(InObject)
    ...
    // Creates a shared reference as a shared reference to an existing shared reference after 
    ...
    // Used internally to create a shared reference from an existing shared reference
...
// SharedPtr is a non-intrusive reference counted authoritative object pointer
template<class ObjectType>
class SharedPtr
{
public:
    ...
    // Constructs a shared pointer as a shared reference to an existing shared pointers object
    ...
    // Allows implicit conversion from a SharedRef to a SharedPtr because it's always safe
    template<class OtherType>
...
public:
    ...
    // Object must have not expired, ensure SharedPtr is valid before using
...
// Weak object pointers with MakeShareabale to allow the them to be implicitly
