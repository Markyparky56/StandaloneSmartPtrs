...
public:
    // Default Constructor - Initialises to null
    inline UniquePtr()
        : Ptr(nullptr)
    {
    }

    // Pointer constructor - Takes ownserhsip of the pointed-to object
    explicit inline UniquePtr(T* InPtr)
        : Ptr(InPtr)
    {
    }

    // Nullptr constructor - Initialises  to null
    ...
    inline UniquePtr& operator=(UniquePtr&& Other)
    {
        if (this != &Other)
        {
            ...
            // if the destructor of T relies on the state of this or Other
            T* OldPtr = Ptr;
            Ptr = Other.Ptr;
            Other.Ptr = nullptr;
            delete OldPtr;
    ...
    // Assigned operator for rvalues of other (usually derived) types
    template< typename OtherT >
...
// If variadic templates aren't available these overloads will support passing up to
