...
// ESPMode is only there to act as a dummy to stop some classes breaking
// WeakReferencer and SharedReferencer depend on each other, causing compilation
// Issues which are avoided by one of them being a template (Hacky, but thread
// safety was not something I was interested in at the time of writing this file)
